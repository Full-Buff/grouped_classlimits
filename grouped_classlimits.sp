...
void LoadGroupLimitsConfig()
{
    ...
    if (kv.GotoFirstSubKey(false)) // false = don't create key if missing
    {
        do
        {
            ...
            {
                // Get section name into a buffer and compare
                char sectionName[64];
                kv.GetSectionName(sectionName, sizeof(sectionName));
                
                if (strcmp(sectionName, "classes") == 0)
                {
                    ...
                    if (kv.GotoFirstSubKey(false))
                    {
                        ...
                        kv.GoBack(); // Go back from classes list to the class subkey itself
                    }
                    else
                    {
                ...
                kv.GoBack(); // Go back from the subkey to the group key
            }
            else
            {
                LogMessage("Group '%s' is missing any subkeys. Group will have no effect.", groupName);
            }
